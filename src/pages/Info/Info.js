      ...
      <Content>
        <Text>
          <h4 style={{ marginBottom: 10 }}>
            1. Ссылка на репозиторий с базовым решением
          </h4>
          <a href="https://gitlab.com/gochamp1/gohackathon-base-client-instance">
            https://gitlab.com/gochamp1/gohackathon-base-client-instance
          </a>
          <p style={{ marginBottom: 15 }}></p>
          <h4 style={{ marginBottom: 10 }}>2. Описание задачи</h4>
          <p>
            Необходимо разработать интерфейс, с помощью которого игрок в Го
            может взаимодействовать с ИИ в ходе игры. Следует придумать способ
            эффективного донесения информации до пользователя с целью победы с
            максимумом набранных очков. Участники сами решают, как и какая
            информация будет сообщаться пользователю и придумывают
            дополнительные функции и механики, которые можно реализовать,
            используя аналитику от Leela Zero. Во избежание ситуации прямого
            получения человеком подсказки от ИИ создана система биллинга.
            Система биллинга учитывает запросы к ИИ на подсказки и штрафует за
            это. Повторюсь, необходимо за меньшее количество подсказок победить
            в игре. Выбор хода в любом случае остаётся за человеком. Хакатон по
            фронтенд разработке, генерации и реализации пользовательских фич на
            основе, заранее созданной технической базы. Данная постановка задачи
            приближена к процессам разработки в индустрии. Участники выступают в
            ролях: разработчиков интерфейсов, продуктовых аналитиков, проектных
            и продуктовых менеджеров. Ниже приводится описание технической базы
            на основе которой будет создаваться участниками хакатона приложение
          </p>
          <p style={{ marginBottom: 15 }}></p>
          <h4 style={{ marginBottom: 10 }}>
            3. Описание токенов для создания своего клиента на базе базового
            решения
          </h4>
          <ol>
            <li>
              1.Токен (аргумент token), получаемый участником при регистрации.
              Он уникален для каждого пользователя и не изменяется при повторных
              аутентификациях. Потребуется для взаимодействия с большинством
              методов API и в процессе общения с сервером через веб-сокет.
            </li>
            <li>
              2. Кентавр-токен (аргумент centaur_token). Выдается каждой
              команде, потребуется для запроса подсказок от сервера по API.
            </li>
          </ol>
          <p style={{ marginBottom: 15 }}></p>
          <h4 style={{ marginBottom: 10 }}>
            4. Ссылка с подписью на документацию Swagger и объяснение, что там
            можно проверить запросы
          </h4>
          <a href="https://go-backend-denis.ambersoft.llc/api/doc">
            https://go-backend-denis.ambersoft.llc/api/doc
          </a>
          <p style={{ marginBottom: 15 }}></p>
          Удобная Swagger-документация к API. Там вы сможете не только
          познакомиться со всеми доступными методами, но и протестировать их.
          Для этого раскройте нужную вам вкладку с описанием и в правом верхнем
          углу нажмите “Try it out”. Далее, после заполнения тела запроса,
          нажмите “Execute”. Ответ сервера не заставит себя долго ждать!
          <p style={{ marginBottom: 15 }}></p>
          ...
          При авторизации на сервере одновременно открывается непрерывное
          соединение через веб-сокет<p style={{ marginBottom: 15 }}></p>
          ws://172.104.137.176:41239/. Описание данных, передаваемых по сокетам
          - От пользователя на сервер Для того, чтобы принимать или отправлять
          данные по сокету необходимо выполнить три шага (пример кода на js):
          <p style={{ marginBottom: 15 }}></p>
...
`}</pre>
          ...
          - От сервера пользователю Оповещение о нашем подключении к партии
          <pre>
            {`
          payload: {
...
`}
          ...
          <h4 style={{ marginBottom: 10 }}>
            5. Таблица со стоимостями базовых методов и система биллинга
          </h4>
          Базовые методы представляют собой Тьюринг-полное пространство. С
          помощью базовых методов есть возможность создавать более сложные
          сценарии. Например: Сценарий “Выживание в углу” Используемые базовые
          методы: В какой 1/4 доски лучший ход Как будет развиваться игра на
          2,3,4 хода вперед.
          <ol>
            ...
            <li>
              13. Тепловая карта всей доски. Детализированная (стоимость - 2)
            </li>
            <li>
              13. Тепловая карта всей доски. Менее детализированная (стоимость -
              2)
            </li>
            ...
            <li>
              17. В какой четверти доски будет лучший ход соперника? (стоимость
              - 2)
            </li>
            <li>
              18. В какой половине доски будет лучший ход соперника? (стоимость
              - 2)
            </li>
            <li>
              19. Показать лучший из заданных 2 ходов соперника (стоимость - 2)
            </li>
            <li>
              20. Показать лучший из заданных 3 ходов соперника (стоимость - 2)
            </li>
            <li>
              21. Показать лучший из заданных 4 ходов соперника (стоимость - 2)
            </li>
            <li>
              22. В какой четверти доски сейчас лучший ход? (стоимость - 1)
            </li>
            <li>
              23. В какой половине доски сейчас лучший ход? (стоимость - 1)
            </li>
            ...
            <li>
              28. Тепловая карта 1-й и 2-й четвертей доски (стоимость - 1)
            </li>
            <li>
              29. Тепловая карта 3-й и 4-й четвертей доски (стоимость - 1)
            </li>
            <li>
              30. Тепловая карта 1-й и 4-й четвертей доски (стоимость - 1)
            </li>
            <li>
              31. Тепловая карта 2-й и 2-й четвертей доски (стоимость - 1)
            </li>
          ...
          <p style={{ marginBottom: 15 }}></p>
          <h4 style={{ marginBottom: 10 }}>
            Задача игры: добиться наилучшего исхода игры при минимальном кол-ве
            баллов на базовые методы.{' '}
          </h4>
          Победитель партии определяется по формуле:
          <p></p>
          <img
            src="https://go-backend-denis.ambersoft.llc/uploads/summ.png"
            alt={'sum'}
          />
          <ol>
